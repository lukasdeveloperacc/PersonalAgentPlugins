...
- `report-result`: create or update the PM-visible `DEVELOPER_REPORT.md` after implementation, verification, blocker, or interrupted work.
...
## PM-Visible Developer Reports

Claude Developer should report durable work status here unless TASK_SPEC says otherwise:

```text
docs/ai-handoffs/<task_id>/DEVELOPER_REPORT.md
```

Codex PM should include this value as `developer_report_path` in every non-trivial TASK_SPEC and include the same path in `allowed_files`. Claude Developer updates the report at start, completion, blocker, partial completion, or verification-only handoff. Codex PM can then run `pm-plugin:pm-sync` to inspect current Developer state without relying on Claude chat history.

The report records:

- selected OMC harness or direct execution path
- files changed
- acceptance criteria evidence
- verification, browser, visual, and OMC evidence
- scope deviations, blockers, residual risk, and PM follow-up needed
...
### Report results for PM sync

```text
/developer-plugin:report-result

TASK_SPEC: docs/tasks/TASK_SPEC-001.md
Developer report path: docs/ai-handoffs/TASK_SPEC-001/DEVELOPER_REPORT.md

현재 변경 파일, 검증 결과, browser evidence, OMC harness 결과를 정리해서
PM이 pm-sync로 볼 수 있는 Developer report를 업데이트해줘.
완료가 아니면 blocked/partial/failed 중 정확한 상태로 남겨줘.
```
...
`docs/task-spec-contract.md` defines the shared TASK_SPEC schema. `docs/design-spec-contract.md` defines the DESIGN_SPEC schema for Claude Designer, Codex Reviewer, and Claude Developer. `docs/developer-report-contract.md` defines the PM-visible Developer result report schema. Installed plugins also carry plugin-local contract copies so skills can run after installation without reading repository-level docs.
