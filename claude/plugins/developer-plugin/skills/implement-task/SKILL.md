...
Use plugin-local `contracts/developer-report-contract.md` to create or update the PM-visible Developer report.
...
5. Confirm `developer_report_path` and that the same path is in `allowed_files` for non-trivial work. If missing, block before implementation or record an explicit low-risk exception.
6. If required TASK_SPEC or DESIGN_SPEC fields are missing or contradictory, report the blocker before implementation.
7. Create or update the Developer report with `developer_status: in_progress` before non-trivial implementation.
8. Select the execution harness:
...
9. Use only one primary loop authority. Do not start Ralph, Team, UltraQA, and Ultragoal as competing loops.
10. Implement the smallest safe change inside scope, or invoke the selected OMC harness and follow its state/exit rules.
11. Add or update tests according to the TASK_SPEC.
12. For browser/UI work, use Chrome DevTools MCP for console/network/DOM/screenshot evidence. Use `/oh-my-claudecode:visual-verdict` when reference screenshots or design evidence exist.
13. Run targeted checks first, then broader lint/typecheck/test commands when available. Use `/oh-my-claudecode:verify` for final evidence-only verification and `/oh-my-claudecode:ultraqa` for repeat diagnose/fix cycles.
14. Update the Developer report before final response with status, changed files, acceptance criteria evidence, verification evidence, blockers, residual risk, and PM follow-up needed.
15. Report changed files, harness decision, Developer report path, verification evidence, screenshots or visual evidence when relevant, and residual risk.
...
- Do not leave non-trivial work without a PM-visible Developer report. If the report cannot be written, include its full content in the final response and mark the missing file as a blocker.
...
Include path, status, and whether it was updated.
